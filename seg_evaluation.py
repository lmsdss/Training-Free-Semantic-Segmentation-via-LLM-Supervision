...
def zero_shot_classifier(model, classnames, make_template, tokenizer, ENV):
    with torch.no_grad():
        ...
        for classname in tqdm(classnames):
            ...
            # [80,25,768]
            class_embeddings = model.module.forward_text_feature(input_ids, attention_mask)  # model.module.

            class_embeddings = model.module.forward_text_project(class_embeddings, attention_mask)  # model.module.
...
def evaluate_benchmark(loader, model, cfg, tokenizer, seg_categories, top_cls_num, subclass_path):
    ...
    sub_list_lengths = []
    for key, value in sub2super.items():
        if isinstance(value, list):
            sub_list_lengths.append(len(value))

    sub_classes, reset_raw_to_super_mapping = get_CLIP_inputs_from_dict(sub2super, seg_categories)


    label_text_feature = zero_shot_classifier(model, seg_categories, openai_imagenet_template, tokenizer, ENV)
    # [231，512]
    label_text_feature_sub = zero_shot_classifier(model, sub_classes, openai_imagenet_template, tokenizer, ENV)

    count = 0
    total_intersection, total_union = 0, 0

    pngindex = 0  # Replace this with the desired index
    ...
    for batch in tqdm(loader):
        ...
        image_feature = model.module.forward_image_feature(batch_dict['image'])  # (Batch, 324, 768)  model.module.
        # projector and L2 normalize
        image_feature_pooled = model.module.forward_image_project(image_feature)  # (Batch, 512) model.module.
        image_feature = model.module.image_projection(image_feature)  # (Batch, 324, 512) model.module.
        ...
        im_f_a = F.normalize(im_f_a, dim=-1, p=2)  # [324,512]
        im_avg_a = image_feature_pooled[index]  # [512]

        i2t_scores = torch.sum(im_avg_a.unsqueeze(0) * label_text_feature, dim=1)
        scores = i2t_scores

        topk_scores, topk_index = scores.topk(int(top_cls_num))  # top_cls_num
        ...
        for i, index in enumerate(topk_index[:candidate_class_num]):
            ...
            index_range = sub_list_lengths[index]
            start_index = sum(sub_list_lengths[:index])
            end_index = start_index + index_range
            selected_text_feature_sub = label_text_feature_sub[start_index:end_index]

            sub_score = torch.sum(im_avg_a.unsqueeze(0) * selected_text_feature_sub, dim=1)  # [11]
            sub_softmax = torch.nn.functional.softmax(sub_score, dim=0)  # [11]
            weighted_feature = torch.mm(sub_softmax.view(1, -1), selected_text_feature_sub)  # [1,512]

            attn_ai2at = im_f_a @ weighted_feature.unsqueeze(-1)  # (324, 1)
...
def parse_args():
    ...
    parser.add_argument("--subclass_path", type=str, default='')
    parser.add_argument("--top_cls_num", type=str, default='')
...
def main():
    ...
    if 'image_encoder.model.model.pos_embed' in checkpoint['state_dict']:
        pos_embed_reshaped = interpolate_pos_embed(checkpoint['state_dict']['image_encoder.model.model.pos_embed'],
                                                   model.module.image_encoder.model.model)  # model.image_encoder    /  model.module
    ...
    for name in cfg.data.valid_name:
        ...
        with torch.no_grad():
            evaluate_benchmark(valid_loader, model, cfg, tokenizer, seg_categories, args.top_cls_num, args.subclass_path)
